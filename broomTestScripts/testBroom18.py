...
class PulseIVSweep:
    ...
    def setup_pulse(self):
        ...
        time.sleep(0.35)
        self.send_command('SOUR:PDEL:WIDT 0.0000005')
        time.sleep(0.35)
        self.send_command('SOUR:PDEL:SDEL 0.0000005')
        time.sleep(0.35)
    ...
    def perform_sweep(self):
        ...
        if range(len(self.U)) == 0:
            print("No data returned from output.")
            return []
        if int(str(range(len(self.U)))) < 11:
            print("Not enough data points returned from output. Querying again.")
            data = str(self.k6221.query('SYST:COMM:SER:ENT?')).strip().strip('[]')
            datalist = data.split(',')[::2]
            # Filter out non-numeric data
            datalist = re.findall(r"[-+]?\d*\.\d+|\d+", data)
            ...
            print("Data points returned from output.")
            pass
